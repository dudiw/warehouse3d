...
export class PathPointList {
  constructor() {
    ...
    this.helpVec3_1 = new THREE.Vector3();
    this.helpVec3_2 = new THREE.Vector3();
    this.helpVec3_3 = new THREE.Vector3();
    this.helpMat4 = new THREE.Matrix4();
    this.helpCurve = new THREE.QuadraticBezierCurve3();
  }
   ...
   */
  set(points, cornerRadius, cornerSplit, up, close) {
  ...
  _corner(current, next, cornerRadius, cornerSplit, up) {
    if (cornerRadius > 0 && cornerSplit > 0) {
      ...
      var curve = this._getCornerBezierCurve(lastPoint.pos, current, next, cornerRadius, this.helpCurve);
  ...
  _sharpCorner(current, next, up, dirType, sharp) {
    var lastPoint = this.array[this.count - 1];
    var point = this._getByIndex(this.count);

    var lastDir = this.helpVec3_1.subVectors(current, lastPoint.pos);
    var nextDir = this.helpVec3_2.subVectors(next, current);
    ...
    if (up) {
      ...
      var vec = this.helpVec3_3.crossVectors(lastPoint.dir, point.dir);
      if (vec.length() > Number.EPSILON) {
        vec.normalize();
        var theta = Math.acos(Math.min(Math.max(lastPoint.dir.dot(point.dir), -1), 1)); // clamp for floating pt errors
        point.up.applyMatrix4(this.helpMat4.makeRotationAxis(vec, theta));
      }
  ...
  _end(current) {
    ...
    point.up.copy(lastPoint.up);
    var vec = this.helpVec3_1.crossVectors(lastPoint.dir, point.dir);
    if (vec.length() > Number.EPSILON) {
      vec.normalize();
      var theta = Math.acos(Math.min(Math.max(lastPoint.dir.dot(point.dir), -1), 1)); // clamp for floating pt errors
      point.up.applyMatrix4(this.helpMat4.makeRotationAxis(vec, theta));
  ...
  _getCornerBezierCurve(last, current, next, cornerRadius, out) {
    var lastDir = this.helpVec3_1.subVectors(current, last);
    var nextDir = this.helpVec3_2.subVectors(next, current);
...
export class PathGeometry extends THREE.BufferGeometry {
  ...
  _resizeAttribute(name, attribute, len) {
...
export class PathTubeGeometry extends PathGeometry {
  constructor(maxVertex, generateUv2) {
    super(this, maxVertex || 1000, generateUv2);
  }
